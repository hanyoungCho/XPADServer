...
type
  ...
  public
    ...
    procedure SetCmdSendBuffer(AFloorNm, ATeeboxNm, AType: String);
...
begin
  ...
  begin
    ...
    FHeatInfoList[nHeatNo].FloorNm := Copy(rTeeboxInfo.FloorNm, 1, 1);
...
begin
  ...
  sStr := 'FloorNm: ' + FHeatInfoList[nHeatNo].FloorNm + ' / TeeboxNm: ' + FHeatInfoList[nHeatNo].TeeboxNm + ' / ' + AType;
  ...
  if ACtrl = True then
    SetCmdSendBuffer(FHeatInfoList[nHeatNo].FloorNm, FHeatInfoList[nHeatNo].TeeboxNm, AType);
end;
  ...
  begin
    ...
    begin
      ...
      if (Global.ADConfig.StoreCode = 'A8004') or //A8004	쇼골프 도봉점
         (Global.ADConfig.StoreCode = 'A6001') then //캐슬렉스
        sSendData := sSendData + ',' + FHeatInfoList[nIndex].FloorNm + '^' + FHeatInfoList[nIndex].TeeboxNm + '^0'
      else
...
procedure TTcpThreadHeat.SetCmdSendBuffer(AFloorNm, ATeeboxNm, AType: String);
var
  sSendData: AnsiString;
begin
  ...
  if (Global.ADConfig.StoreCode = 'A8004') or //A8004	쇼골프 도봉점
     (Global.ADConfig.StoreCode = 'A6001') then //캐슬렉스
    sSendData := 'SBS,' + AFloorNm + '^' + ATeeboxNm + '^' + AType
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      SetCmdSendBuffer(FHeatInfoList[nHeatNo].FloorNm, FHeatInfoList[nHeatNo].TeeboxNm, '0');

      sStr := '자동히터정지 - 층: ' + FHeatInfoList[nHeatNo].FloorNm + ' / Nm: ' + FHeatInfoList[nHeatNo].TeeboxNm + ' / ' +
  ...
  begin
    try
      ...
      begin //대기중인 제어명령이 있으면
        ...
        //Global.Log.LogHeatWrite(sSendData);
