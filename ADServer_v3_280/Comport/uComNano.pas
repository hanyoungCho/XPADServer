...
begin
  ...
  if nStx > nEtx then
  begin
    // 그랜드 - %04$WD16 / 25 30 34 24 57 44 31 36 0D -> 일부타석 제어시 응답값 들어옴. 이후 응답값 일부 짤려서 $OD 들어오는 경우 있음.
    FRecvData := Copy(FRecvData, nStx, length(FRecvData)-nEtx );
    Global.Log.LogReadMulti(FIndex, 'nStx > nEtx : ' + FRecvData + ' / ' + StrToAnsiHex(FRecvData));
    Exit;
  end;
  ...
  if (Length(FRecvData) <> 33) then
  begin
    //Global.Log.LogReadMulti(FIndex, 'FRecvData fail : ' + IntToStr(nStx) + ' / ' + IntToStr(nEtx));
    Global.Log.LogReadMulti(FIndex, 'FRecvData fail : ' + FRecvData + ' / ' + StrToAnsiHex(FRecvData));
    FRecvData := '';
    Exit;
  end;
...
begin
  ...
  begin
    ...
    begin
      if Global.ADConfig.StoreCode = 'BD001' then //	BD001	그랜드골프클럽 //시간변경
      begin
