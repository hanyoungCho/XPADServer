...
type
  ...
  public
    ...
    function TeeboxHeatUseUpdate(AStoreCode, ATeeboxNo, AHeatUse, AHeatAuto, AUserId: String): AnsiString;
    function TeeboxHeatUseDomeUpdate(AStoreCode, ATeeboxNms, AHeatUse, AHeatAuto: String): AnsiString; //돔골프 전용-히터가 여러타석 제어시
    ...
    //주차권 발행
    function SeatUseParkSelect(AStoreCode, AReserveNo: String): String;
    function SeatUseParkUpdate(AStoreCode, AReserveNo: String): String;

    //agent
    function SelectAgent(AAgentNo, ALeftHanded: String): String;
    function InsertAgent(AAgentNo, ALeftHanded, ASetting: String): Boolean;
    function UpdateAgent(AAgentNo, ALeftHanded, ASetting: String): Boolean;
...
begin
  ...
  try
    ...
    begin
      if (Global.ADConfig.ProtocolType = 'ZOOM') or (Global.ADConfig.ProtocolType = 'JMS') or
    ...
    try
      try
        ...
        begin
          ...
          if (global.ADConfig.StoreCode = 'B7001') then //프라자 3층 제외, 53번부터
            sSql := sSql + ' AND SEAT_NO < 53';

          if (global.ADConfig.StoreCode = 'A8004') then //쇼골프(도봉점), 53번부터
            sSql := sSql + ' AND SEAT_NO < 53';
...
function TXGolfDM.TeeboxHeatUseUpdate(AStoreCode, ATeeboxNo, AHeatUse, AHeatAuto, AUserId: String): AnsiString;
var
  sSql: String;
  sStr, sLog: String;
begin
  ...
  try
    ...
    begin
      ...
      sLog := 'TeeboxHeatUseUpdate Exception: ' + E.Message;
      Global.Log.LogErpApiWrite(sLog);
    end;
  end;
...
function TXGolfDM.TeeboxHeatUseDomeUpdate(AStoreCode, ATeeboxNms, AHeatUse, AHeatAuto: String): AnsiString; //돔돌프 전용-히터가 여러타석 제어시
var
  sSql: String;
  sStr, sLog: String;
begin
  //fan, heat 동일컬럼 사용 -> 히터만 이구문 사용
...
function TXGolfDM.SeatUseParkSelect(AStoreCode, AReserveNo: String): String;
var
  sSql: String;
  sUseSeqDate: String;
  nUseSeqNo: Integer;
begin
  ...
  sUseSeqDate := Copy(AReserveNo, 1, 8);
  nUseSeqNo := StrToInt(Copy(AReserveNo, 9, 4));
  ...
  begin
    ...
    try
      try
        sSql :=  ' select * from seat_use ' +
                 ' where store_cd = ' + QuotedStr(AStoreCode) +
                 ' and use_seq_no = ' + IntToStr(nUseSeqNo) +
                 ' and use_seq_date = ' + QuotedStr(sUseSeqDate);
        ...
        if FieldByName('RESERVE_ROOT_DIV').AsString <> 'M' then
        begin
          Result := '주차권 발행 대상이 아닙니다.';
          Exit;
        end;

        //1:일일타석, 2:기간회원, 3:쿠폰회원
        if (FieldByName('RESERVE_DIV').AsString = '2') then
        begin
          Result := '주차권 발행 대상이 아닙니다.(기간회원)';
          Exit;
        end;

        if FieldByName('PARK_PRINT').AsString <> 'N' then
        begin
          Result := '주차권이 이미 발행되었습니다.';
          Exit;
        end;
...
function TXGolfDM.SeatUseParkUpdate(AStoreCode, AReserveNo: String): String;
var
  sSql, sUseSeqDate, sUseSeqNo: String;
  nNo: Integer;
  sLog: String;
...
function TXGolfDM.SelectAgent(AAgentNo, ALeftHanded: String): String;
var
  sSql: String;
begin
...
function TXGolfDM.InsertAgent(AAgentNo, ALeftHanded, ASetting: String): Boolean;
var
  sSql: String;
  I: Integer;
begin
...
function TXGolfDM.UpdateAgent(AAgentNo, ALeftHanded, ASetting: String): Boolean;
var
  sSql: String;
  I: Integer;
begin
