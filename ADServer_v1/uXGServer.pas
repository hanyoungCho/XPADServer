...
type
  ...
  public
    ...
    function SetTeeboxUsed(AReceiveData: AnsiString): AnsiString; //타석기배정내역
    ...
    function BallRecallTimeCheck(ASeatUseInfo: TSeatUseInfo): Boolean; //볼회수 시간 배정시간에 추가
...
begin
  ...
  try
    try
      ...
      if sApi = '9901' then //2022-03-15 모바일AD 체크용
      begin
      ...
      begin
        if (Global.ADConfig.StoreCode = 'T0001') or //장한평
           (Global.ADConfig.StoreCode = 'T0002') or //장한평테스트
           (Global.ADConfig.StoreCode = 'A8001') or //쇼골프(SHOW GOLF)
           (Global.ADConfig.StoreCode = 'B2001') then //그린필드골프연습
        begin
          //
        end
        else
      ...
      else if sApi = 'K414_TeeBoxHeat' then   //히터
...
begin
  ...
  try
    ...
    (* 2021-06-04 쇼골프에서 볼회수시 문제발생 체크로직 예외처리
...
begin
  ...
  try
    try
      ...
      // 2021-10-13 키오스크에서 배정완료후 홀드취소 보냄. 배정완료인 경우 AD에서 홀드 취소처리함.  // adserver 2022-07-20 적용
...
begin
  ...
  try
    ...
    //2020-08-18 제휴사코드 추가
    //if (Global.ADConfig.ProtocolType = 'JMS') and (Global.ADConfig.StoreCode = 'A3001') then //2021-04-21 주석처리
    ...
    begin
      ...
      if (Global.ADConfig.StoreCode = 'B2001') or //그린필드골프연습
         (Global.ADConfig.StoreCode = 'T0001') or //장한평
         (Global.ADConfig.StoreCode = 'A8001') or //쇼골프
         (Global.ADConfig.StoreCode = 'T0002') then //장한평 테스트
      begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          // 사용일자 미표시 결정-2021-03-29 이종섭과장 (JV as TJSONArray).Items[i].P['coupon']
...
function TTcpServer.SetTeeboxUsed(AReceiveData: AnsiString): AnsiString; //타석기배정내역
begin
...
begin
  ...
  try
    ...
    //if (Global.ControlComPortHeatMonThread = nil) and //스타
    if (Global.TcpThreadHeat = nil) then //그린필드
    begin
...
begin
  ...
  try
    ...
    if (Global.ADConfig.StoreCode = 'T0001') or //장한평
       (Global.ADConfig.StoreCode = 'A1001') or //스타:현재 서버버전
       (Global.ADConfig.StoreCode = 'A2001') or //양평
...
begin
  ...
  try
    ...
    begin
      if sMode = 'N' then
        Result := '{"result_cd":"AD01","result_msg":"fail. Oauth 인증을 확인해주세요"}'
      else
...
var
  ...
  //jSendItemObj: TJSONObject; // pos,kiosk 보낼전문
  //jSendObjArr, jSendSubObjArr: TJSONArray;
...
begin
  ...
  try
    ...
    if (Global.ADConfig.StoreCode = 'B2001') or //그린필드골프연습
       (Global.ADConfig.StoreCode = 'T0001') or //장한평
       (Global.ADConfig.StoreCode = 'A8001') or //쇼골프(SHOW GOLF)
       (Global.ADConfig.StoreCode = 'T0002') then //장한평테스트
    begin
      if (sReserveDiv = '2') and
...
begin
  ...
  try
    ...
    begin
      Result := '{"result_cd":"0010",' +
                 '"result_msg":"' + sMemberNm + ' 님 모바일로 예약된 배정이 없습니다."}';
...
function TTcpServer.BallRecallTimeCheck(ASeatUseInfo: TSeatUseInfo): Boolean; //볼회수 시간 배정시간에 추가
var
...
begin
  ...
  sStr := '볼회수 체크 : ' + ASeatUseInfo.ReserveNo + ' / sSTm: ' + sSTm + ' / sETm: ' +sETm;
