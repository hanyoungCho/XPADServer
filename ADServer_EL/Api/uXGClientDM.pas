...
type
  ...
  public
    ...
    function TeeboxUseUpdate(AUserId, ATeeboxNo, AErrorDiv: String): AnsiString; //사용여부
    ...
    function SetSeatReserveTmChange(AStoreCode, ASeatNo, AUseSeqDate, ADelayTm: String): String; //예약시간 증가
  ...
  begin
    ...
    try
      ...
      begin
        ...
        rSeatInfo.FloorCd := FieldByName('FLOOR_ZONE_CODE').AsString;
        //rSeatInfo.ZoneLeft := FieldByName('SEAT_ZONE_CODE').AsString;
        rSeatInfo.ZoneDiv := FieldByName('SEAT_ZONE_CODE').AsString;
...
begin
  ...
  try
    ...
    sSql := ' UPDATE SEAT SET ' +
            ...
            '     FLOOR_ZONE_CODE = ' + QuotedStr(ATeeboxInfo.FloorCd) + ',' +
...
function TXGolfDM.TeeboxUseUpdate(AUserId, ATeeboxNo, AErrorDiv: String): AnsiString;
var
  sSql, sLog: String;
begin
  Result := '';
  ...
  begin
    ...
    try
      try
        ...
        sSql := ' UPDATE SEAT ' +
                ' SET USE_YN = ' + QuotedStr(AErrorDiv) + ',' +
                '     REG_DATE = now() ' +
                ' WHERE STORE_CD = ' + QuotedStr(Global.ADConfig.StoreCode) +
                ' AND SEAT_NO = ' + ATeeboxNo;
    ...
    try
      try
        ...
        sSql :=  ' update seat_use set ' +
               ...
               '  , MEMBER_SEQ = ' + QuotedStr(ASeatUseInfo.MemberSeq) +
               '  , MEMBER_NM = ' + QuotedStr(ASeatUseInfo.MemberNm) +
  ...
  try
    ...
    try
      ...
      sSql := ' select ' +
            ...
            '   and ( (now() > reserve_date ) or (now() > start_date) ) '; //2022-07-27 즉시배정된 배정
